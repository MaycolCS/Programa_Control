<?php
    ...
    if (isset($_POST['Malla_Peso'])) {
        if (esta_Malla($Malla_Tipo, $Malla_Hueco, $Malla_Calibre)) {
            if (eliminar_Malla($Id_Malla)) {
                header("Location: ../MainPage? cc=$Documento&cs=$CS&msj=OBM");
                exit();
            } else {
                echo '<script>alert("La malla no se pudo eliminar, intentelo nuevamente")</script>';
            }
        } else {
            $Malla_Tipo = 0;
            $Malla_Hueco = 0;
...
<html>
    ...
    <body>
        ...
        <section>
            ...
            <?php
            if ($Malla_Tipo == 0) {?>
                <form class="form_Style" method="post" action="EliminarMalla? cc=<?php echo $Documento; ?>&cs=<?php echo $CS; ?>&MALTIP=<?php echo $Malla_Tipo; ?>&MALHUE=<?php echo $Malla_Hueco; ?>&MALCAL=<?php echo $Malla_Calibre; ?>">
                    <p class="txt_Titulo">Eliminar malla</p>
                    <div>
                        <label>Tipo:</label>
            ...
            <?php
                ...
                <form class="form_Style" method="post" action="EliminarMalla? cc=<?php echo $Documento; ?>&cs=<?php echo $CS; ?>&MALTIP=<?php echo $Malla_Tipo; ?>&MALHUE=<?php echo $Malla_Hueco; ?>&MALCAL=<?php echo $Malla_Calibre; ?>">
                    ...
                    <div>
                        ...
                        <input readonly=»readonly» type="number" name="Malla_Peso" id="Malla_Peso" min="0" step="any" placeholder="Total de peso" value="<?php echo $Datos_Malla[3]; ?>" autocomplete="off" required/>
                    </div>
                    <div>
                        <label>Horas (hrs/M<sup>2</sup>):</label>
                        <input readonly=»readonly» type="number" name="Malla_Horas" id="Malla_Horas" min="0" step="any" placeholder="Cantidad de horas" value="<?php echo $Datos_Malla[4]; ?>" autocomplete="off" required/>
                    </div>
                    <div>
                        <label>Precio:</label>
                        <input readonly=»readonly» type="number" name="Malla_Precio" id="Malla_Precio" min="1" step="any" placeholder="Precio actual" value="<?php echo $Datos_Malla[5]; ?>" autocomplete="off" required/>
                    </div>
                    <div class="Boton_Style">
