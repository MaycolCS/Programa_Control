<?php
    ...
    if (isset($_POST['Malla_Peso'])) {
        ...
        if (!esta_Malla($Malla_Tipo, $Malla_Hueco, $Malla_Calibre)) {
            if (insertar_Malla($Malla_Tipo, $Malla_Hueco, $Malla_Calibre, $Malla_Peso, $Malla_Horas, $Malla_Precio, $Documento)) {
                header("Location: ../MainPage? cc=$Documento&cs=$CS&msj=OGM");
                exit();
            } else {
            ...
            $Malla_Hueco = 0;
            $Malla_Calibre = 0;
            echo '<script>alert("La malla ya se encuentra registrada en la base de datos")</script>';
        }
    }
...
<html>
    ...
    <body>
        ...
        <section>
            ...
            <?php
            if ($Malla_Tipo == 0) {?>
                <form class="form_Style" method="post" action="IngresarMalla? cc=<?php echo $Documento; ?>&cs=<?php echo $CS; ?>&MALTIP=<?php echo $Malla_Tipo; ?>&MALHUE=<?php echo $Malla_Hueco; ?>&MALCAL=<?php echo $Malla_Calibre; ?>">
                    <p class="txt_Titulo">Registro malla</p>
                    <div>
                        <label>Tipo:</label>
            ...
            <?php
                ...
                <form class="form_Style" method="post" action="IngresarMalla? cc=<?php echo $Documento; ?>&cs=<?php echo $CS; ?>&MALTIP=<?php echo $Malla_Tipo; ?>&MALHUE=<?php echo $Malla_Hueco; ?>&MALCAL=<?php echo $Malla_Calibre; ?>">
                    ...
                    <div>
                        ...
                        <input type="number" name="Malla_Peso" id="Malla_Peso" min="0" step="any" placeholder="Total de peso" autocomplete="off" required/>
                    </div>
                    <div>
                        <label>Horas (hrs/M<sup>2</sup>):</label>
                        <input type="number" name="Malla_Horas" id="Malla_Horas" min="0" step="any" placeholder="Cantidad de horas" autocomplete="off" required/>
                    </div>
                    <div>
                        <label>Precio:</label>
                        <input type="number" name="Malla_Precio" id="Malla_Precio" min="1" placeholder="Precio actual" autocomplete="off" required/>
                    </div>
                    <div class="Boton_Style">
            ...
            if (count(lista_Mallas()) > 0) {
                ...
                <div id="tabla_vistaEscritorio">
                    <table class="tabla_encabezado">
                        <tr>
                            <td class="datos_fijos"># malla</td>
                            <td class="datos_fijos">Tipo malla</td>
                            <td class="datos_fijos">Hueco malla</td>
                            <td class="datos_fijos">Calibre malla</td>
                        </tr>
                        <?php
                        for ($i = 0; $i < count($Mallas); $i += 4) {?>
                            <tr>
                                <td class="datos_variables"><?php echo ($i/4+1);?></td>
                                <td class="datos_variables"><?php echo $Mallas[$i+1];?></td>
                                <td class="datos_variables"><?php echo $Mallas[$i+2];?></td>
                                <td class="datos_variables"><?php echo $Mallas[$i+3];?></td>
                            </tr>
                        <?php
                        }
                        ?>
                ...
                <div id="lista_WR">
                    <ul>
                        <?php
                        for ($i = 0; $i < count($Mallas); $i += 4) {?>
                            <li># malla: <?php echo ($i/4+1);?></li>
                            <ul>
                                <li>Tipo malla: <?php echo $Mallas[$i+1];?></li>
                                <li>Hueco malla: <?php echo $Mallas[$i+2];?></li>
                                <li>Calibre malla: <?php echo $Mallas[$i+3];?></li>
                            </ul>
                        <?php
                        }
                        ?>
