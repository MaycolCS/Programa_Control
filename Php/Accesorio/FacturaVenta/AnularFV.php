<?php
    ...
    $FacturasVenta = listaFacturasVentaSinAnular();
    $cant_FacturasVenta = count($FacturasVenta);
    $aux_cant_FacturasVenta = 0;
    if ($cant_FacturasVenta == 0) {
        header("Location: ../MainPage.php? cc=$Documento&cs=$CS&msj=LSFPA");
        exit();
    }
    ...
    if (isset($_POST['FV'])) {
        ...
        if (!estaFacturaVenta($IdFacturaVenta)) {
            header("Location: AnularFV? cc=$Documento&cs=$CS&msj=ESFV");
            exit();
        } else {
...
<html>
    ...
    <body>
        ...
        <section>
            ...
            <?php
            if ($IdFacturaVenta=="") {?>
                <form class="form_Style" method="post" action="AnularFV.php? cc=<?php echo $Documento; ?>&cs=<?php echo $CS; ?>">
                    <p class="txt_Titulo">Anular factura</p>
                    <div>
                        <label>Factura:</label>
            ...
            <?php
                    ...
                    <table class="tabla_footer">
                        ...
                        <tr>
                            ...
                            <td class="datos_variables" rowspan="3">
                                El valor debe ser consignado en la cuenta de ahorros a nombre de </br>
                ...
                <?php
                    if ($DatosFV['facturaventa_Anulada'] != "SI") {?>
                        <form name="form" class="form_Style" method="post" action="AnularFVDB.php? cc=<?php echo $Documento; ?>&cs=<?php echo $CS; ?>&FV=<?php echo $DatosFV['facturaventa_Id']; ?>">
                            <div>
                                <label>Motivo:</label>
                                <textarea type="text" name="MotivoAnulacion" id="MotivoAnulacion" autocomplete="off" required></textarea>
                            </div>    
                            <div>
                                <button type="submit">Anular factura</button>
                            </div>
                        </form>
                    <?php
                    }?>
